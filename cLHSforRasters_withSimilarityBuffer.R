...
buffer <- similarity_buffer(r.stack.sub, s2$sampled_data[1,], buffer= bufDist, fac=15) #15 is index of geomorphons
plot(buffer)
...
for (i in 2:numPoints){
  ...
  r.stack.sub <- crop(r.stack, ext)
  buffer <- similarity_buffer(r.stack.sub, s2$sampled_data[i,], buffer= bufDist, fac=15) #15 is index of geomorphons
